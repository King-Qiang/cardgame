...
-- 表结构说明
-- ============================================================
-- player              玩家主表（设计文档中的 user，避免 MySQL 保留字）
-- user_profile        用户扩展（等级、VIP）
-- user_wallet         钱包（金币/钻石，乐观锁）
-- user_ban            封禁记录
-- admin_role          运营角色
-- admin_user          运营账号
-- operation_log       操作审计
...
-- room_player         房间玩家
-- game_record         对局记录
-- game_action_log     对局操作流水（回放）
-- game_settlement     对局结算
-- wallet_transaction  钱包流水
-- shop_item           商城商品
-- recharge_order         充值订单（设计文档中的 order，避免保留字）
-- wallet_adjust_request  调账审批
-- activity_config        活动配置
-- system_config          系统配置
-- player_sign_record     签到记录
-- player_rank            玩家段位
-- player_rank_log        段位变动日志
-- ============================================================
...
CREATE TABLE IF NOT EXISTS `wallet_adjust_request` (
...
CREATE TABLE IF NOT EXISTS `player_sign_record` (
    `id`           BIGINT      NOT NULL AUTO_INCREMENT COMMENT '主键',
    `user_id`      BIGINT      NOT NULL COMMENT '用户 ID',
